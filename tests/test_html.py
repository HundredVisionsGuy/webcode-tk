...
@pytest.fixture
def sports_h1():
    tag = html.get_elements("h1", sports_path)
    return tag[0]


@pytest.fixture
def div_list_tag():
    tag = html.get_elements("div", no_inline_styles)
    return tag[0]
...
def test_get_html_for_file_not_found():
    with pytest.raises(FileNotFoundError):
        html.get_html("file/to/non-existent.html")
...
def test_get_num_elements_in_file_for_FNF_exception():
    with pytest.raises(FileNotFoundError):
        html.get_num_elements_in_file("p", "non/existent/file.html")


def test_get_num_elements_in_nonexistent_folder():
    with pytest.raises(FileNotFoundError):
        html.get_num_elements_in_folder("p", "folder/path")


def test_get_num_elements_in_test_project_folder():
    num_paragraphs = html.get_num_elements_in_folder("p", project_path)
    assert num_paragraphs == 6


def test_get_elements_for_file_not_found_exception():
    with pytest.raises(FileNotFoundError):
        html.get_elements("p", "non/existent/file.html")


def test_get_elements_for_expected_number():
    results = html.get_elements("ul", sports_path)
...
def test_get_element_content_for_single_el_as_string():
    ...
    results = html.get_element_content(tag_string)
    assert results == expected


def test_string_to_tag_for_div_markup():
    results = html.string_to_tag(div_markup)
